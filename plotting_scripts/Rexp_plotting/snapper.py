...
'VOLTAGE_0.15-0.6000000000000001V', 'VOLTAGE_0.15-0.7V', 'VOLTAGE_0.15-0.8V']
	...
	for i in range(2,7):
		...
		for sing_snap in data[0][0]:
			j+=1
			filtred_snap = filterr(sing_snap, 40)	

			######SNAP#############################################
			# new_down_liner(filtred_snap, 300)
			plt.ylim(0,15000)
			plt.plot(sing_snap, color='b')
			ns_up = new_up_liner(sing_snap, 4000)
			# for l in ns_up:
			# 	plt.plot([l for i in sing_snap])
			ns_down = new_down_liner(filtred_snap, 200)
			if len(ns_up)!=0:
				plt.plot([s_average(ns_up) for i in sing_snap])
			if len(ns_down)!=0:
				plt.plot([s_average(ns_down) for i in sing_snap])
