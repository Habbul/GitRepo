...
def averlines_plotting():
	for i in range(1,7):
		...
		# with open('resistance_plot.txt', 'w') as f:
		# 	f.write(str(resistance_plot))
		resistance_plot = filterr(resistance_plot, 10)
	...
	plt.ylim(0, 5)
	plt.legend(tuple(['Amplitude 0.15-{}V'.format(i) for i in input_up_lines_files[6::]]))
	plt.xlabel('Time, min')
