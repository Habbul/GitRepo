...
def averlines_plotting():
	...
	for i in range(0,2):
		...
		resistance_plot = [1.2/(i/10873/abs(input_voltage-0.15)-1) for i in filtred_amp_plot]
	...
	# plt.ylim(0, 5)
	plt.legend(tuple(['{}V'.format(i) for i in amps[0::]]))
	plt.xlabel('Time, min')
